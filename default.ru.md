...
В случае если в RCML передаются траектори для роботов "хардкорно" - методом пост-процессинга (прописывание координат в коде программы):
...
Событийно-ориентированный подход не очевиден в RCML, однако используется почти во всех задачах, где есть потребность реакции на возникновение события. При этом источников событий, как и ответных действий на них может быть множество.

Рассмотрим пример: робот должен остановиться при нажатии на кнопку подключенную к внешнему блоку сигналов подключенному по Modbus, робот должен перенести деталь по сигналу с датчика подключенному к роботу.
Имеем 2 типа события: нажатие на кнопку останова и датчик наличия детали ожидающей переноса.
