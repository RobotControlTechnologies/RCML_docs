...
6. Зарезервированные слова, перечисленные в разделе ["Перечень зарезервированных ключевых слов"](#12-1).
...
- *system.mutex_lock* – производит попытку блокировки мютекса с именем указанным в качестве строкового параметра. Если мютекс свободен блокирует его. Если такой мютекст ещё не определен, то создает и блокирует его. Подробнее в разделе ["Блокировки"](#10);
- *system.mutex_unlock* – снимает блокировку мютекса с именем указанным в качестве строкового параметра. Если мютекс уже свободен, бросывает исключение. Если такой мютекст ещё не определен, то создает свободный мютекст с данным именем;
...
В процессе работы интерпретатора *RCML*, если была включена опция записи статистики (см. раздел ["Установка и настройка компилятора и интерпретатора RCML"](#13rcml)), накапливается статистическая информация о работе роботов и скорости выполнения ими своих функций, некоторые модули выбора робота корректно работают только при включенной опции записи статистики, в частности модули *avg*.
...
Простейшим примером необходимости применения блокировок, когда два и более независимых друг от друга робота берут или приносят некие детали в одно и тоже место, чтобы не допустить столкновения роботов необходимо запретить входить в данную общую зону пространства любому другому роботу, если хотя бы один робот уже находиться в общей зоне.

Для управления блокировками в RCML используется два понятия: мтютексы и семафоры.

### 10.1 Мтютексы

Мютекст - механизм ограничвающий работу нескольких паралельно выполняемых функций (потоков) над одним участком кода.

Некоторые из проблем, которые могут решать мютексы:
...
Мютекс имеет уникальное имя и может находиться всего в двух состояниях: заблокирован или свободен.
Заблокированный ранее мютекст нельзя заблокировать повторно, таким образом выполнение всех участков кода, которые пытаются работать с одним и тем же мютексом, приостанавливается пока мютекст не будет свободен.

Блокировка мютекста выполняется через системную функцию *system.mutex_lock*. Она имеет всего один параметр - название мютекса (строка). Если мютекст с таким именем уже существует, то выполнение данного участка кода будет приостановлено до разблокировки мютекста.

Чтобы разблокировать мютекс необходимо вызвать системную функцию *system.mutex_unlock*. Она также имеет всего один параметр - название мютекса. После разблокировки мютекса он становится свободным для повторного блокирования и только кто-то один из ожидающих может его заблокировать. 

**Порядок блокировки мютекса ожидающими участками случаен.**

Пример использования мютексов:
```cpp
@r = robot_builder;
...
В данном примере с помощью мютексов создаются критические секции - секции кода, которые могут выполняться только одним исполнителем в текущий момент времени. При создании критических секций рекомендуется минимизировать код попадающий в критическую секцию и производить освобождение мютекса в той же функции, в которой он был заблокирован. 

### 10.2 Семафоры

В RCML семафор является механизмом для генерации событий от одного или нескольких источников к одному или нескольким получателям.

Семафор также как и мютекс имеет уникальное имя, но в отличиее от мютекса имеет счетчик освобождений. 
Чтобы создать семафор необходимо вызвать системную функцию *system.semaphore_create_lock*. Она имеет два параметра:
- название семафора - строка;
- количество освобождений - число - сколько раз должен быть освобожден семафор, до того как он сгенерирует событие.

Если семафор с таким именем уже создан, то будет брошено исключение. Семафор будет существовать пока счетчик освобождений больше нуля.

Ожидание выполнением освобождения семафора происходит через функцию *system.semaphore_wait*, имя ожидаемого семафора указывается в качестве единственного строкового параметра. Когда счетчик освобождений достигнет нуля, выполнение продолжиться с этой функции, до этого текущий поток будет находиться в состоянии сна.
...
Функции *system.semaphore_wait* и *system.semaphore_unlock* генерируют исключение если семафор с указанным именем не существует.

Пример генерации события от множества к одному:
```
...
В данном примере главная функция ждем завершения работы дочерних функций по установке блоков, которые управляют множеством роботов. Переход к постройке следующего уровня осуществляется только после завершения предудущего.

Пример генерации события от одного потока к множеству. Дополним предыдущий пример возможностью одновременно старта постройки трех блоков:
```
function mountBlock(level_num) {
...
# 12 Перечень зарезервированных ключевых слов
...
# 13 Синтаксическая карта языка RCML
```cpp
...
# 14 Подробнее об интерфейсных идентификаторах применительно к библиотекам
...
Создайте свой аккаунт в сервисе [Репозиторий](https://repository.rcml.info/#/sign-up)
...
Перейдите в секцию [«deployment keys» - ключи развертывания](https://repository.rcml.info/#/keys/deployment) и добавьте свой ключ. Ключ можно сгенерировать согласно командам на скриншотах.
...
На каждом ПК и контроллере, выполните следующую команду в консоли: `rcml_intepreter --hardware-info`
...
Повторите пункты 18.2 и 18.3 для всех ПК и контроллеров, на которых необходима работа лицензии.
