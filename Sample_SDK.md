...
- `MoveLineral` осуществляет линейное движение ([LINEAR Interpolated Motion](#LMotion)) робота по декартовым координатам.
- `MoveCartesian` осуществляет осевое движение ( [JOINT Interpolated motion](#JMotion)) робота по декартовым координатам.
- `MoveJoint` осуществляет движение ([JOINT Interpolated motion](#JMotion)) осей робота до заданного значения углов поворота осей.
- `MoveArc` осуществляет движение по дуге ([CIRCULAR Interpolated Motion](#CMotion)).  

<a name="JMotion">JOINT Interpolated Motion:</a>
...
<a name="LMotion">LINEAR Interpolated Motion:</a>
...
<a name="CMotion">CIRCULAR Interpolated Motion</a>

- Передвижение TCP, из начальной в конечную точку, происходит по дуге.
- Для построения дуги необходимо указать дополнительную (промежуточную) точку.
